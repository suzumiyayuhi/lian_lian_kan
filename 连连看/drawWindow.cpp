...
int			g_curve,g_cx,g_cy,g_x1,g_x2,g_y1,g_y2;
...
void DRAWWINDOW::DrawGameMap(HWND hWnd, int timeLength, int score)
{
	HDC hdc;
	hdc = GetDC(hWnd);
	...
	CImage image;													//ÔØÈë±³¾°Í¼
	image.Load(TEXT("res\\interface\\game.jpg"));
...
void DRAWWINDOW::DrawSelected(int x, int y, int val, bool mark)
{
	if (mark && val != 0)
	{
		HDC memDC = CreateCompatibleDC(m_hdc);
...
void DRAWWINDOW::DrawInterface(const TCHAR* picPath)
{
	HDC bgDC = CreateCompatibleDC(m_hdc);
	BITMAP temBmp;
...
bool DRAWWINDOW::IfDelete(int x1, int y1, int v1, int x2, int y2, int v2)
{
	if (v1 != v2)
		return false;
	if (x1 == x2&&y1 == y2)
...
bool DRAWWINDOW::sol(int x, int y)
{
